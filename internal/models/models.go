...
// User is the type for transactions
type User struct {
	ID        int       `json:"id"`
...
func (m *DBModel) GetWidget(id int) (Widget, error) {
	...
	row := m.DB.QueryRowContext(ctx, getWidgetSql, id)
	err := row.Scan(
		&widget.ID,
		&widget.Name,
		&widget.Description,
...
func (m *DBModel) InsertTransaction(txn Transaction) (int, error) {
	...
	result, err := m.DB.ExecContext(ctx, insertTransactionSql,
		txn.Amount,
		txn.Currency,
		txn.LastFour,
		txn.BankReturnCode,
...
func (m *DBModel) InsertOrder(order Order) (int, error) {
	...
	result, err := m.DB.ExecContext(ctx, insertOrderSql,
		order.WidgetID,
		order.TransactionID,
		order.StatusID,
		order.Quantity,
...
func (m *DBModel) InsertCustomer(c Customer) (int, error) {
	...
	result, err := m.DB.ExecContext(ctx, insertCustomerSql,
		c.FirstName,
		c.LastName,
		c.Email,
		time.Now(),
...
func (m *DBModel) GetUserByEmail(email string) (User, error) {
	...
	row := m.DB.QueryRowContext(ctx, getUserByEmailSql, email)
...
func (m *DBModel) Authenticate(email, password string) (int, error) {
	...
	row := m.DB.QueryRowContext(ctx, authenticateUserSql, email)
...
const updataPasswordForUserSql = `
...
func (m *DBModel) UpdatePasswordForUser(u User, hash string) error {
	...
	_, err := m.DB.ExecContext(ctx, updataPasswordForUserSql, hash, u.ID)
	if err != nil {
		return err
	}
	return nil
